<script>document.cookie = "sw="+window.innerWidth;</script>
				...
				<div class="row">
					...
					<div class='auth col-lg-4 col-md-4 col-sm-10 col-xs-12'>
						...
						<select v-model="regionName">
							<option v-for="name in single_option" v-bind:value="name">{{ name }}</option>
						</select><br><br>

						<span>код регіону</span><br>
						<select v-model="regionKod">
							<option v-for="option in symbols" v-bind:value="option">{{ option }}</option>
						</select><br><br>
